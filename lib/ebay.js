...
// Maschera il token nei log.
function sanitizeXml(xml) {
  return xml.replace(/<eBayAuthToken>[\s\S]*?<\/eBayAuthToken>/g, '<eBayAuthToken>***</eBayAuthToken>');
}

function logRequest(callName, xml) {
  console.log(`[ebay] → ${callName}\n${sanitizeXml(xml)}`);
}

function logResponse(callName, httpStatus, text) {
  const ack = xmlText(text, 'Ack');
  if (ack === 'Success') {
    console.log(`[ebay] ← ${callName}: Ack=Success (HTTP ${httpStatus})`);
  } else {
    console.log(`[ebay] ← ${callName}: Ack=${ack} (HTTP ${httpStatus})\n${text.slice(0, 3000)}`);
  }
}
...
async function tradingCall(env, callName, innerXml) {
  ...
  logResponse(callName, res.status, text);
...
export async function uploadPicture(env, buffer, name) {
  ...
  logRequest('UploadSiteHostedPictures', `${xml}  [+ immagine ${buffer.length} byte]`);
  const res = await fetch(c.endpoint, {
    method: 'POST',
  ...
  const text = await res.text();
  logResponse('UploadSiteHostedPictures', res.status, text);
  ...
  console.log(`[ebay]   foto caricata: ${url}`);
  return url;
}
...
// Impagina la descrizione per eBay in HTML curato (solo stili inline,
// niente contenuti attivi: è quanto eBay consente).
function descriptionHtml(draft) {
  const wine = '#6b1d2a'; // bordeaux
  ...
  const rows = [
    ...
    .map(
      ([k, v]) =>
        `<tr>` +
        `<td style="padding:9px 14px;border:1px solid #e0d8cc;background:#f7f3ec;font-weight:bold;white-space:nowrap">${esc(k)}</td>` +
        `<td style="padding:9px 14px;border:1px solid #e0d8cc">${esc(v)}</td>` +
        `</tr>`
    )
    .join('\n');

  return `<div style="font-family:Georgia,'Times New Roman',serif;max-width:800px;margin:0 auto;color:#2b2b2b;line-height:1.6;font-size:16px">
  <h2 style="font-family:Arial,Helvetica,sans-serif;font-size:23px;color:${wine};border-bottom:3px solid ${wine};padding-bottom:10px;margin:0 0 18px">${esc(draft.titolo || '')}</h2>
  ${paragraphs}
  ${rows ? `<h3 style="font-family:Arial,Helvetica,sans-serif;font-size:17px;color:${wine};margin:24px 0 10px">Caratteristiche</h3>
  <table style="border-collapse:collapse;width:100%;font-size:15px">${rows}</table>` : ''}
  <div style="margin-top:22px;padding:14px 18px;background:#fdf6e3;border-left:5px solid #c9a227;font-family:Arial,Helvetica,sans-serif;font-size:14px">
    <b>Bottiglia venduta come pezzo da collezione:</b> conservata in ambiente domestico a temperatura non controllata, l'eventuale contenuto non è destinato al consumo. Vendita riservata ai maggiorenni.
  </div>
  <p style="margin:18px 0 0;font-style:italic;color:#6a6a6a;font-size:14px">Le foto fanno parte integrante della descrizione: osservale con attenzione prima dell'acquisto.</p>
</div>`;
}
...
export async function publishListing(env, draft, pictureUrls) {
  ...
  // Proposte d'acquisto: sotto la soglia minima eBay rifiuta da solo,
  // sopra quella di accettazione compra-subito automatico.
  const bestOffer = (env.EBAY_BEST_OFFER || 'true').toLowerCase() !== 'false';
  const offerMinPct = Number(env.EBAY_OFFER_MIN_PERCENT || '50');
  const offerAcceptPct = Number(env.EBAY_OFFER_AUTOACCEPT_PERCENT || '80');
  const bestOfferXml = bestOffer
    ? `<BestOfferDetails><BestOfferEnabled>true</BestOfferEnabled></BestOfferDetails>` +
      `<ListingDetails>` +
      `<MinimumBestOfferPrice currencyID="EUR">${((price * offerMinPct) / 100).toFixed(2)}</MinimumBestOfferPrice>` +
      `<BestOfferAutoAcceptPrice currencyID="EUR">${((price * offerAcceptPct) / 100).toFixed(2)}</BestOfferAutoAcceptPrice>` +
      `</ListingDetails>`
    : '';
